        ...
        if (Input.touchCount > 0 && !matched)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - nosePlace.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - nosePlace.y) <= 0.5f)
                    {
                        transform.position = new Vector2(nosePlace.x, nosePlace.y);
                        matched = true;
                    }
                    else
                    {
