        ...
        if (Input.touchCount > 0 && !matched)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - eyesPlace.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - eyesPlace.y) <= 0.5f)
                    {
                        transform.position = new Vector2(eyesPlace.x, eyesPlace.y);
                        matched = true;
                    }
                    else
                    {
