        ...
        if (Input.touchCount > 0 && !matched)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - mouthPlace.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - mouthPlace.y) <= 0.5f)
                    {
                        transform.position = new Vector2(mouthPlace.x, mouthPlace.y);
                        matched = true;
                    }
                    else
                    {
