        ...
        if (Input.touchCount > 0 && !matched)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - leftPlace.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - leftPlace.y) <= 0.5f)
                    {
                        transform.position = new Vector2(leftPlace.x, leftPlace.y);
                        matched = true;
                    }
                    else
                    {
