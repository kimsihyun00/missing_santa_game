        ...
        if(Input.touchCount > 0 && ! matched)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(transform.position.x - hatPlace.x) <=0.5f &&
                        Mathf.Abs(transform.position.y - hatPlace.y) <=0.5f)
                    {
                        transform.position = new Vector2(hatPlace.x, hatPlace.y);
                        matched = true;
                    }
                    else
                    {
